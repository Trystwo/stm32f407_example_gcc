 ...
 * @retval      无
 */
SRAM_HandleTypeDef g_sram_handler; /* SRAM句柄, 需要定义为全局变量*/
...
void sram_init(void)
{
    ...
    /* PG0~5,10 */
    gpio_init_struct.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_10;
...
void sram_read(uint8_t *pbuf, uint32_t addr, uint32_t datalen)
{
    for (; datalen != 0; datalen--)
    {
        *pbuf++ = *(uint8_t *)(SRAM_BASE_ADDR + addr);
        addr++;
    }
}
